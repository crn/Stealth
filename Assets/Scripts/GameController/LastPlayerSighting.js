...
public var musicFadeSpeed : float = 1f;                                 // The speed at which the music fades.
...
private var sirens : AudioSource[];                                     // Reference to the AudioSource of the megaphones.
...
function SwitchAlarms(){
    ...
    // If the position is not the reset position then set the new intensity to low.
    ...
    // For all sirens: if alarm is triggered and the audio isn't playing, then play the audio. Otherwise, stop the audio.
    for(var i = 0; i < sirens.Length; i++){
...
function MusicFading(){
    ...
    // If the alarm is not being triggered fade out the normal music and fade in the panic music.
    if(position != resetPosition){
